...
> then runs Phases 1–4 against them. Phase 5 is a manual walkthrough and cannot be
...
1. Frontmatter (TT-19): extend `description` (≤ 1,024 characters) to mention tier
   ...
   `metadata.version: "2.0"` and the new `metadata.harny-writes`. Leave the
...
2. Regenerate exactly the five contracted golden files per tree (TT-24). Scaffold both
   golden scenarios with the built CLI (`node bin/harness.js init …`) into scratch
   `git init` directories. Copy only the five paths into
   `tests/fixtures/golden/monorepo-mode/ts-root/` and
   `tests/fixtures/golden/monorepo-mode/py-sub/apps/api/`. Then confirm with
   `git status --short tests/fixtures/golden` that exactly 8 modified files and 2 new
...
### Phase 5: Manual demo walkthrough (workshop rehearsal)
**Goal**: Verify the prompt-level runtime behavior that automated tests cannot verify,
and rehearse the 2026-09-27 workshop demo (TT-25, intent G6, SC10).
...
1. **Sample repo**: in a scratch directory outside this repository, create a small
   ...
   (and optionally a second tool, for example Cursor).
...
5. **M4, missing framework and no install before yes**: in M1 or M3, before
   ...
   `CONFIRMED`.
6. Record each walkthrough's outcome (date, tool, pass/fail, deviations, transcript or
   screenshot path) in `audit.md` rows M1–M4.
...
| Golden regeneration sweeps in unrelated drift | Low | Med | TT-24: copy only five paths per tree. The existing path-set and byte comparison catches any other change |
...
| Behavior differs across the five tools (for example, a tool whose sub-agents cannot be resumed) | Med | Med | TT-15 falls back to a fresh invocation with context. Only Claude Code (plus one optional second tool) is walked through, and the rest is carried as TT-R2, alongside AL-30 and CG-1 |
...
- `tests/test-writer-templates.test.ts` — CREATE — TT-5 citations, TT-17 coupling, TT-16 gate count and preserved strings, TT-2 rubric section structure
- `tests/skills-placement.test.ts` — MODIFY — per-tool placement of every `harny-test` file (TT-1)
- `tests/skills-fidelity.test.ts` — MODIFY — `DIVERGENCE_TABLE['harny-test']` (TT-22)
...
- `tests/canonical-fidelity.test.ts` — MODIFY — T41 allowlist entry `templates/roles/sdd-test-writer.md` (TT-24)
- `tests/fixtures/golden/monorepo-mode/ts-root/.claude/skills/harny-test/high-value-tests.md` — CREATE (regenerated)
- `tests/fixtures/golden/monorepo-mode/ts-root/.claude/{skills/harny-test/SKILL.md,agents/sdd-test-writer.md,skills/sdd-conductor/SKILL.md,skills/README.md}` — MODIFY (regenerated)
- `tests/fixtures/golden/monorepo-mode/py-sub/apps/api/.claude/skills/harny-test/high-value-tests.md` — CREATE (regenerated)
- `tests/fixtures/golden/monorepo-mode/py-sub/apps/api/.claude/{skills/harny-test/SKILL.md,agents/sdd-test-writer.md,skills/sdd-conductor/SKILL.md,skills/README.md}` — MODIFY (regenerated)
- `specs/test-tiers/audit.md`, `specs/test-tiers/tasks.md` — MODIFY — tracking, including M1–M4 results
