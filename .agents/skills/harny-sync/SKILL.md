...
2. Move `specs/<feature>/` to `specs/archived/<feature>/` as a whole directory. **Check
   ...
   state — do not assume one. (Per `contract.md` § Amendment A2: now that `specs/` is
   fully tracked, most archive moves will find the source tracked, but a feature archived
   before its first commit is still untracked, and both must work.)
