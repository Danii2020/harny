...
describe('runDoctor — pre-flight, exit-code translation, and no-write proof (T3, T4, T5)', () => {
   ...
   * Covers: contract.md § State Changes ("src/** — UNCHANGED"), § Dependencies
   * ("src/doctor.ts ... not modified"); Behavior Guarantee RC-6; audit.md
   * Test Coverage T10; tasks.md Task 1.10R.
   *
   * `src/doctor.ts` is not touched by this feature at all (RC-6), so this is
   * a declared continuity guard, the same "expected to pass already at red
   * time" posture this repo's other regression guards document explicitly
   * (see e.g. `tests/canonical-fidelity.test.ts`'s ci-workflow-root
   * golden-copy block) — not a pending implementation gap. It exists so a
   * future change to `runDoctor`'s spawn call accidentally forwarding
   * `--only` is caught as a regression rather than discovered downstream.
   */
  it('never passes --only to the runner — only ["--checks", <json>], exactly as before this feature (RC-6) (T10)', async () => {
    const { runDoctor, DOCTOR_RUNNER_PATH } = await import('../src/doctor.js');
    const targetDir = await makeScaffoldedTarget(
      [
    ...
    const argvPath = path.join(targetDir, path.dirname(DOCTOR_RUNNER_PATH), 'argv.json');
    const argv = JSON.parse(await fs.readFile(argvPath, 'utf8'));

    expect(argv).not.toContain('--only');
    expect(argv[0]).toBe('--checks');
  });
});

/**
 * Spec: specs/documentation-role-completion
 * Covers: contract.md § Data Models ("`.sdd/doctor/checks.json` is not
 * regenerated by this feature"); Behavior Guarantee RC-6; audit.md Test
 * Coverage T11; tasks.md Task 1.10R.
 *
 * Same declared-continuity-guard posture as the test immediately above: this
 * feature adds no `checks.json` field, so this passes already and exists to
 * catch a stray `only`/`selector` key creeping in as a regression.
 */
describe('.sdd/doctor/checks.json is unchanged by this feature — no new selector-related field (RC-6) (T11)', () => {
  it('declares exactly the pre-feature top-level and specs key sets', async () => {
    const raw = await fs.readFile(path.join(REPO_ROOT, '.sdd', 'doctor', 'checks.json'), 'utf8');
    ...
    expect(Object.keys(parsed).sort()).toEqual(
      ...
      ['commands', 'repoReadiness', 'repoReadinessLabel', 'require', 'security', 'securityLabel', 'specs', 'version'].sort(),
    );
